...
# Create an instance of the FieldStorage class to parse the CGI request
form = cgi.FieldStorage()
# Get the value of the "name" parameter from the GET request
link = form.getvalue('url')
file_format = form.getvalue('format')
...
filename_ = re.split(r"[/\\]",list_files[0])[-1]
print("Content-Disposition: attachment; filename=" + filename_)

if file_format == 'bestaudio':
    print("Content-type: audio/mpeg")

# Read the video file content and send it to the client
