        ...
        try:
            ...
            if message.startswith("https://www.youtube.com/watch?v="):
                nativemessaging.send_message(
                    nativemessaging.encode_message("Processing, please wait..."))
                ...
                if getattr(sys, 'frozen', False):
                    ...
                    # Command to run the subprocess
                    subprocess_args = [os.path.join(
                        script_dir, "ytd_subprocess.exe")]
                else:
                    ...
                    # Command to run the subprocess
                    subprocess_args = ["python", os.path.join(
                        script_dir, "ytd_subprocess.py")]

                subprocess_obj = subprocess.Popen(
                    subprocess_args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=False)

                # Communicate with the subprocess
                stdout_byte, stderr_byte = subprocess_obj.communicate(
                ...
                if result[0] == 'Success':
                    nativemessaging.send_message(
                        nativemessaging.encode_message("Finished"))
                    ...
                    notification.notify(
                        ...
                        # The notification will automatically close after 10 seconds (optional)
                        timeout=10,
                    )
                else:
